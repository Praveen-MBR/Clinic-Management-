...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        const response = await axios.get("http://localhost:8080/api/lab-reports");
  ...
  const handleDelete = async (id) => {
    try {
        await axios.delete(`http://localhost:8080/api/lab-reports/${id}`);
        ...
        <tbody>
          {reports.map((report) => (
            <tr key={report.id}>
              ...
              <td>
                <button onClick={() => handleDelete(report.id)}>Delete</button>n
              </td>
            </tr>
          ))}
        </tbody>
